    ...
    with open(label_file) as f_label:
       ...
       for line in lines:
          ...
          if line[2] != 'DontCare':
             ...
             center = center[0] 
             bb_list.append([frame_index, center, box3d_corner, category])
    ...
    for n in range(len(pcd_frames)):
       ...
       for k in range(len(bb_list)):
          if int(bb_list[k][0]) == n : 
             bboxes.append([bb_list[k][1], bb_list[k][2], bb_list[k][3]]) 
             
             if flag:
                x_min = np.min(bb_list[k][2][:, 0]) 
                x_max = np.max(bb_list[k][2][:, 0])
                y_min = np.min(bb_list[k][2][:, 1])
                y_max = np.max(bb_list[k][2][:, 1])
                z_min = np.min(bb_list[k][2][:, 2])
                z_max = np.max(bb_list[k][2][:, 2])
                ...
                mask_x = np.logical_and(x <= x_max, x >= x_min) 
                mask_y = np.logical_and(y <= y_max, y >= y_min)
                mask_z = np.logical_and(z <= z_max, z >= z_min)
                ...
                mask = np.logical_and(np.logical_and(mask_x, mask_y), mask_z) 
                cropped_points = np.vstack((cropped_points, points[mask]))
