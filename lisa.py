...
velo_frame_path = "/home/parvez/temp_folder/training/velodyne/000000.bin"
...
with open(velo_frame_path, "rb") as f:
      ...
      while byte:
         ...
         clear_data.append([x,y,z,intensity])
         byte = f.read(size_float * 4)
      
points = np.asarray(list_pcd) 
data_clear = np.asarray(clear_data)
...
# Get render options and set point size and background color
opt = vis.get_render_option()
opt.background_color = np.asarray([0, 0, 0])  # Black background
opt.point_size = 1.0                          # Smaller point size (default is 5.0)

vis.run()
vis.destroy_window()"""

print("data_clear", data_clear.shape)
data_r33 = lisa.augment(data_clear, 33) # rain rate 33 mm/hr 

points = data_r33[:, 0:3]
print("noise data =", data_r33.shape)
#points_aug = np.vstack((points, data_r33[:, 0:3])) 

#print("aug points = ", points_aug.shape)
